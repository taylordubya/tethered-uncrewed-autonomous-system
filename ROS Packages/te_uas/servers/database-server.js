...
app.get("/flights", (req, res) => {
  ...
  if (active === "true") {
    ...
    const activeFlight = db.data.flights.find(
      (flight) => flight.active === true
    );
    if (activeFlight) {
...
app.post("/logs", (req, res) => {
  const { level, description, name } = req.body; // Extract log details from request body
    ...
    if (name) {
      ...
      activeFlight.logs.push({
        logId: generateUniqueId(
          new Set(db.data.flights.map((flight) => flight.id))
...
app.post("/flight", (req, res) => {
  ...
  if (action === "end") {
    if (activeFlight) {
      ...
      db.write(); // Write changes to the database
      res.status(200).json({
        message: "Flight mode changed and logged successfully",
        flight: activeFlight,
      });
    } else {
      res
        .status(404)
        .json({ message: "No active flight found to change mode" }); // No active flight found
    }
    ...
    if (logId) {
      ...
      if (log) {
        if (!log.dismissed.includes(name)) {
          ...
          db.write(); // Write changes to the database
          res.status(200).json({
            updatedLogs: activeFlight.logs,
            message: "Log dismissed successfully",
          });
        } else {
          res
            .status(400)
            .json({ message: "Log already dismissed by this user" }); // Log already dismissed
        }
      } else {
      ...
      res.status(200).json({
        updatedLogs: activeFlight.logs,
        message: "All logs dismissed successfully for the user",
      });
    }
  } else {
    res.status(404).json({ message: "No active flight found" }); // No active flight found
