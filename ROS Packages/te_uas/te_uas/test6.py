...
class DroneControl(Node):
    ...
    def adjust_alt_cb(self, request, response):
        ...
        self.achieve_altitude(2.0)
        
        response.success = True
        return response
    # Function to adjust the altitude of the drone
    def achieve_altitude(self, t):
        target_altitude = self.current_pose.pose.position.z + 2
    ...
    def achieve_position(self, distance):
        self.set_target_pose(self.current_pose.pose.position.x + distance, self.current_pose.pose.position.y, self.current_pose.pose.position.z)
        target_position = self.current_pose.pose.position.x + distance
        # Arm drone then set mode to OFFBOARD
        if not self.current_state.armed:
            self.arm_drone()
        if self.current_state.mode != "OFFBOARD":
        ...
        def position_adjustment_callback():    
            self.local_pos_pub.publish(self.target_pose)

            # When the drone is within 0.3 meters of the target altitude, stop publishing and set mode to AUTO.LOITER
            if abs(self.current_pose.pose.position.x - target_position) <= 0.3:
                self.get_logger().info('Position achieved')
                self.custom_log("INFO", f"Position achieved at {round(self.current_pose.pose.position.x, 3)}m")
                self.set_mode("AUTO.LOITER")
                self.position_timer.cancel()
                self.altitude_timer.cancel() # Stop the timer

        timer_period = 0.025  # Adjust as needed
        self.position_timer = self.create_timer(timer_period, position_adjustment_callback)
